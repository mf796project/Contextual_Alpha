...
def find_IC_cont(constit,rets,factor,context):
    ...
    for i in range(len(times)-1):
        ...
        IC_low.append(np.corrcoef(rts[increa_index_low],fac[increa_index_low])[0,1])
        IC_mid.append(np.corrcoef(rts[increa_index_mid],fac[increa_index_mid])[0,1])
        IC_high.append(np.corrcoef(rts[increa_index_high],fac[increa_index_high])[0,1])
